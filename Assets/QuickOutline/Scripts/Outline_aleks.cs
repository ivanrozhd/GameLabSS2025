...
public class Outline_Aleks : MonoBehaviour {
  ...
  [SerializeField] public TextMeshPro interactionText;
  ...
  // Stuff added so its visible only when in line of sight
  // and added distance trigger
  ...
  void OnEnable() {
    SetOutlineVisibility(false); // Start with outline hidden
  }
  ...
  void Update() {
    ...
    if (enableByDistance && distance <= maxVisibleDistance) {
      ...
      if (Physics.Raycast(ray, out RaycastHit hit, maxVisibleDistance)) {
    ...
    // Update outline visibility if needed
  ...
  public void ShowOutline(bool visible)
  {
    ...
    isVisible = visible; // prevents Update from overriding it
    if (interactionText != null)
